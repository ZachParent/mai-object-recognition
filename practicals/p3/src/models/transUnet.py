...
def get_r50_vit_b16_config_dict():
    ...
    config["resnet"] = {"num_layers": (3, 4, 9), "width_factor": 1}
...
class SegmentationHead(nn.Sequential):
    def __init__(
        self, in_channels, out_channels, kernel_size=3, upsampling=1, activation=None
    ):
        conv2d = nn.Conv2d(
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        for i in range(len(decoder_channels)):
            ...
            block = DecoderBlock(
                in_channels_cup[i],
                out_channels_cup[i],
                skip_channels=skip_ch_for_this_block,
            )
    ...
    def forward(self, hidden_states, features_skip=None):
        ...
        for i, decoder_block in enumerate(self.blocks):
            skip_to_use = None
            if (
                features_skip is not None
                and i < self.config["n_skip"]
                and i < len(features_skip)
            ):
...
class TransUNet(nn.Module):
    def __init__(
        ...
        if config_name not in CONFIGS:
            raise ValueError(
                f"Config {config_name} not found. Available: {list(CONFIGS.keys())}"
            )
        ...
        self.transformer = Transformer(
            self.vit_config, img_size=self.img_size, in_channels=self.in_channels
        )
        ...
        if Path(pretrained_path).is_file():
            ...
            try:
                ...
                print(
                    f"ERROR loading NPZ weights: {e}. Model weights may be partially loaded or random."
                )
        else:
            print(
                f"WARNING: Pretrained weights not found at {pretrained_path}. Model initialized randomly."
            )
        ...
        with torch.no_grad():
            ...
            if self.load_resnet_weights:
                if self.transformer.embeddings.hybrid:
                    print(
                        "Attempting to load ResNet weights (load_resnet_weights=True, model is hybrid)."
                    )
                    ...
                    for bname, block_module in hybrid_model.body.named_children():
                        for uname, unit_module in block_module.named_children():
                            ...
                            if first_key_check in weights_np:
                                unit_module.load_from(
                                    weights_np, n_block=bname, n_unit=uname
                                )
                                resnet_body_loaded_any = True
                    if resnet_body_loaded_any:
                        print(
                            "ResNet body weights loaded (or attempted for relevant units)."
                        )
                    else:
                        print(
                            "WARNING: No ResNet body unit weights were loaded (keys might be missing for all units)."
                        )

                else:  # Not hybrid
                    print(
                        "NOTE: Skipping ResNet weights: Model is not hybrid, though load_resnet_weights=True."
                    )
            ...
            if "Transformer/posembed_input/pos_embedding" in weights_np:
                posemb_orig = np2th(
                    weights_np["Transformer/posembed_input/pos_embedding"]
                )
                ...
                if n_tok_orig == int(math.sqrt(n_tok_orig - 1)) ** 2 + 1:
                    print(
                        f"Original position embedding has {n_tok_orig} tokens. Assuming one is a CLS token. Stripping it."
                    )
                    posemb_to_resize = posemb_orig[:, 1:]
                elif n_tok_orig == int(math.sqrt(n_tok_orig)) ** 2:
                    print(
                        f"Original position embedding has {n_tok_orig} tokens, forming a square grid."
                    )
                else:
                    logger.warning(
                        f"Original position embedding token count ({n_tok_orig}) is unusual. Proceeding without stripping CLS explicitly here."
                    )

                if posemb_to_resize.shape[1] == n_tok_new:
                    print(
                        "Position embedding token count matches target. Copying directly."
                    )
                    ...
                    logger.info(
                        f"Resizing position embeddings from {posemb_to_resize.shape} to target token count {n_tok_new}"
                    )
                    posemb_grid_squeezed = posemb_to_resize.squeeze(0)

                    if (
                        int(math.sqrt(posemb_grid_squeezed.shape[0])) ** 2
                        != posemb_grid_squeezed.shape[0]
                    ):
                        logger.error(
                            f"Cannot form a square grid from source position embedding of shape {posemb_grid_squeezed.shape} for resizing. Skipping pos_embed loading."
                        )
                        print(
                            "ERROR: Position embedding resizing failed, cannot form square grid from source."
                        )
                    else:
                        ...
                        if gs_old == 0 or gs_new == 0:
                            logger.error(
                                "Grid size old or new is zero. Skipping pos_embed loading."
                            )
                            print(
                                "ERROR: Position embedding resizing failed, grid size zero."
                            )
                        else:
                            print(
                                f"Resizing position embedding grid from {gs_old}x{gs_old} to {gs_new}x{gs_new}."
                            )
                            posemb_reshaped_old = posemb_grid_squeezed.reshape(
                                gs_old, gs_old, -1
                            )
                            zoom_factors = (gs_new / gs_old, gs_new / gs_old, 1)
                            posemb_grid_zoomed = ndimage.zoom(
                                posemb_reshaped_old, zoom_factors, order=1
                            )
                            posemb_final_reshaped = posemb_grid_zoomed.reshape(
                                1, gs_new * gs_new, -1
                            )
                            self.transformer.embeddings.position_embeddings.copy_(
                                torch.from_numpy(posemb_final_reshaped)
                            )
                            print("Transformer position_embeddings resized and loaded.")
            else:
                print(
                    "WARNING: 'Transformer/posembed_input/pos_embedding' not found. Position embeddings initialized randomly."
                )
            ...
            if transformer_blocks_loaded_any:
                print(
                    "Transformer encoder blocks loaded (or attempted for relevant blocks)."
                )
            else:
                print(
                    "WARNING: No Transformer encoder block weights were loaded (keys might be missing)."
                )
