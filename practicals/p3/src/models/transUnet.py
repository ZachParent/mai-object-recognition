...
if __name__ == '__main__':
    if not R50_VIT_B16_PRETRAINED_PATH.exists():
        ...
        block_units = resnet_config['num_layers']
        width_factor = resnet_config['width_factor']
        current_width = int(64 * width_factor)

        for b_idx, num_units_in_block in enumerate(block_units):
            block_cin = current_width if b_idx == 0 else current_width * 2
            if b_idx > 0 : current_width *=2
            
            for u_idx in range(1, num_units_in_block + 1):
                n_block_name, n_unit_name = f"block{b_idx+1}", f"unit{u_idx}"
                unit_cin = block_cin if u_idx == 1 else current_width * 4
                ...
                if u_idx == 1 and ( (b_idx > 0) or (unit_cin != unit_cout) ):
                     dummy_weights.update({
            ...
            if b_idx == (len(block_units)-1):
                 current_width = unit_cout

        dummy_weights.update({
            "embedding/kernel": np.random.randn(1,1,current_width,768).astype(np.float32),
            "embedding/bias": np.random.randn(768).astype(np.float32),
            "Transformer/posembed_input/pos_embedding": np.random.randn(1, 256 + 1, 768).astype(np.float32), # 256 for 16x16 grid, +1 for CLS like token
    ...
    print("\n--- Testing TransUNet with 3 input channels (load_resnet_weights=True) ---")
    model_3ch_load_resnet = TransUNet(config_name='R50-ViT-B_16', img_size=256, in_channels=3, num_classes=1, load_resnet_weights=True)
    dummy_input_3ch = torch.randn(1, 3, 256, 256)
    try:
        output_3ch = model_3ch_load_resnet(dummy_input_3ch)
        print("TransUNet 3ch (load_resnet_weights=True) output shape:", output_3ch.shape)
    except Exception as e:
        print(f"Error during 3ch model test (load_resnet_weights=True): {e}")
        import traceback
        traceback.print_exc()

    print("\n--- Testing TransUNet with 3 input channels (load_resnet_weights=False) ---")
    model_3ch_no_resnet = TransUNet(config_name='R50-ViT-B_16', img_size=256, in_channels=3, num_classes=1, load_resnet_weights=False)
    try:
        output_3ch_no_resnet_w = model_3ch_no_resnet(dummy_input_3ch)
        print("TransUNet 3ch (load_resnet_weights=False) output shape:", output_3ch_no_resnet_w.shape)
    except Exception as e:
        print(f"Error during 3ch model test (load_resnet_weights=False): {e}")
        import traceback
        traceback.print_exc()


    print("\n--- Testing TransUNet with 6 input channels (load_resnet_weights=True, ResNet root should be skipped) ---")
    model_6ch_load_resnet = TransUNet(config_name='R50-ViT-B_16', img_size=256, in_channels=6, num_classes=1, load_resnet_weights=True)
    dummy_input_6ch = torch.randn(1, 6, 256, 256)
    try:
        output_6ch = model_6ch_load_resnet(dummy_input_6ch)
        print("TransUNet 6ch (load_resnet_weights=True) output shape:", output_6ch.shape)
    except Exception as e:
        print(f"Error during 6ch model test (load_resnet_weights=True): {e}")
        import traceback
        traceback.print_exc()

    print("\n--- Testing TransUNet with 6 input channels (load_resnet_weights=False) ---")
    model_6ch_no_resnet = TransUNet(config_name='R50-ViT-B_16', img_size=256, in_channels=6, num_classes=1, load_resnet_weights=False)
    try:
        output_6ch_no_resnet_w = model_6ch_no_resnet(dummy_input_6ch)
        print("TransUNet 6ch (load_resnet_weights=False) output shape:", output_6ch_no_resnet_w.shape)
    except Exception as e:
        print(f"Error during 6ch model test (load_resnet_weights=False): {e}")
        import traceback
        traceback.print_exc()