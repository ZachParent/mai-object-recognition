...
def train_one_epoch(model, train_list, batch_size, n_train_steps, exp):
    ...
    for step in range(n_train_steps):
        ...
        X, Y = next(exp["train_augmentation"].flow(X, Y, batch_size=batch_size, shuffle=False))
    ...
    return train_loss / n_train_steps, train_acc / n_train_steps, train_f1 / n_train_steps
...
def test_one_epoch(model, test_list, batch_size, n_test_steps, exp):
    ...
    for step in range(n_test_steps):
        ...
        X, Y = next(exp["test_augmentation"].flow(X, Y, batch_size=batch_size, shuffle=False))
...
def save_results(exp, train_loss, train_acc, train_f1, test_loss, test_acc, test_f1):
    ...
    with open(results_file, mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["ID", "Test Loss", "Test Accuracy", "Test F1", "Test AUC", "Train Loss", "Train Accuracy", "Train F1", "Train AUC"])
...
def train_and_test(model, exp):
    ...
    for epoch in range(exp["n_epochs"]):
        ...
        train_loss, train_acc, train_f1 = train_one_epoch(model, train_list, exp["batch_size"], n_train_steps, exp)
        train_loss_history.append(train_loss)
        train_acc_history.append(train_acc)
        train_f1_history.append(train_f1)

        print(f"Epoch {epoch} training loss: {train_loss:.2f}, acc: {train_acc:.2f}, f1: {train_f1:.2f}")

        test_loss, test_acc, test_f1 = test_one_epoch(model, test_list, exp["batch_size"], n_test_steps, exp)
        test_loss_history.append(test_loss)
        test_acc_history.append(test_acc)
        test_f1_history.append(test_f1)

        print(f"Epoch {epoch} test loss: {test_loss:.2f}, acc: {test_acc:.2f}, f1: {test_f1:.2f}")

    save_results(exp, train_loss_history[-1], train_acc_history[-1], train_f1_history[-1], test_loss_history[-1], test_acc_history[-1], test_f1_history[-1])

    save_model(model, exp)

    save_history(train_loss_history, train_acc_history, train_f1_history, test_loss_history, test_acc_history, test_f1_history, exp)