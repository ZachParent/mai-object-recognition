...
def generate_balanced_batch_ids(file_list, batch_size=32, min_samples_per_class=1):
    ...
    targets_per_class = {
        cls: (
            max(min_samples_per_class, int((np.sqrt(count) / total_sqrt) * batch_size))
            if count > 0
            else 0
        )
    ...
    if total_target > batch_size:
        ...
        targets_per_class = {
            cls: (
                max(min_samples_per_class, int(target * scale))
                if cls in class_to_files and len(class_to_files[cls]) > 0
                else 0
            )
    ...
    for cls, target in targets_per_class.items():
        ...
        selection = random.sample(cls_files, min(target, len(cls_files)))
...
def create_class_weights(file_list, power=0.5):
    ...
    inv_frequencies = {
        cls: (total_samples / (count + 1)) for cls, count in class_counts.items()
    }

    # Apply power to control weight magnitude
    class_weights = {cls: (freq**power) for cls, freq in inv_frequencies.items()}
