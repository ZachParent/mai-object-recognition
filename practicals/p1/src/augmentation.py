...
def random_erasing_layer(p=0.5, area_range=(0.02, 0.2), aspect_ratio_range=(0.3, 3.0), value=0):
    ...
    def random_erasing(image):
        ...
        # Random aspect ratio
        aspect_ratio = tf.random.uniform((), aspect_ratio_range[0], aspect_ratio_range[1])
        ...
        for i in range(h):
            for j in range(w):
                for k in range(3):
                    indices = indices.write(counter, [top_left_y + i, top_left_x + j, k])
...
def create_data_pipeline(is_training=True, advanced_augmentation=False):
    ...
    augmentation = get_augmentation_pipeline(
        use_augmentation=is_training, 
        advanced_augmentation=advanced_augmentation
    ) if is_training else layers.Identity()
